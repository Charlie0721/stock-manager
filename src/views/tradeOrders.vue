  ...
  <ion-page>
    <ion-header>
      <ion-button
        ...
        @click="checkSales"
        >Consultar ventas del Día</ion-button
      >
      <ion-button
        color="mycolor"
        expand="block"
        class="btn-edit-product"
        @click="paginateOrders(this.idalmacen, this.idvendedor)"
        >Pedidos por vendedor</ion-button
      >
      <ion-toolbar>
        <ion-title>
        ...
        <ion-row>
          <ion-col size="6">
            <ion-button
              ...
              expand="full"
              @click="getCustomers()"
              ><ion-icon :icon="i.peopleCircleOutline"></ion-icon> Cliente
            </ion-button>
          </ion-col>
          <ion-col size="6">
            <ion-button
              ...
              color="mycolor"
              class="btn-edit-product"
              ><ion-icon :icon="i.personCircleOutline"></ion-icon>Vendedor
            </ion-button>
          </ion-col>
        </ion-row>
        <ion-row>
    ...
    <ion-content :fullscreen="true">
      ...
      <ion-text color="medium" v-if="customerName">
        <ion-card>
          <ion-card-content>
            <h3>CLIENTE: {{ customerNit }} {{ customerName }}</h3>
          </ion-card-content>
        </ion-card>
      </ion-text>
      ...
      >
        ...
        <ion-card-content>
          <div style="display: flex; align-items: center">
            <h6 text="dark" style="margin: 0 10px">
              Base $:
              {{
...
export default defineComponent({
  ...
  methods: {
    ...
    async createNeighborhoods() {
      ...
      const newNeighborhood = await TradeOrders.createNeighborhoods(
    ...
    subtractAmount(idproducto: number) {
      const producto = this.productArray.find(
        (r) => r.idproducto === idproducto
      );
      if (producto) {
    ...
    deleteProduct(id: number) {
      try {
        ...
        if (productIndex !== -1) {
          ...
          const deletedTaxValue =
            deletedProduct.cantidad * deletedProduct.taxValue;
    ...
    saveOrderToLocalStorage() {
      // Actualiza el objeto del pedido con los productos actuales
    ...
    recalcularDescuentos() {
      this.valdescuentos = this.productArray.reduce(
        (total, { cantidad, descuento }) => total + descuento * cantidad,
        0
      );
    },
  ...
  computed: {
    subtotal() {
      return this.productArray.reduce(
        (total, { cantidad, valorprod, ivaprod, descuento }) =>
          total + (valorprod - ivaprod - descuento) * cantidad,
        0
      );
    },
    valimpuesto() {
      return this.productArray.reduce(
        (total, { cantidad, valorprod, base }) =>
          total + (valorprod - base) * cantidad,
        0
      );
    },
    total() {
      return this.productArray.reduce(
        (total, { cantidad, valorprod, descuento }) =>
          total + (valorprod - descuento) * cantidad,
        0
      );
    },
    valdescuentos() {
    ...
    computedValorProd(product) {
      return (product) =>
        product.valorprod - (product.valorprod * product.porcdesc) / 100;
    },
    computedTotal() {
